...
public class MonoWavFileReader
{
    ...
    {
        PcmMonoInputStream asis = new PcmMonoInputStream(riffHeaderData.getFormat(), new FileInputStream(file));
    ...
    private void validateFrameBoundaries(int frameStart, int frameEnd)
    {
        ...
        {
            throw new IllegalArgumentException("Start Frame cannot be after end frame. Start:" + frameStart + ", " +
                "end:" + frameEnd);
        }
        if (frameEnd > riffHeaderData.getSampleCount())
        {
            throw new IllegalArgumentException("Frame count out of bounds. Max sample count:" + riffHeaderData
                .getSampleCount() + " but frame is:" + frameEnd);
        }
    }
