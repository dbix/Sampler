...
public class PcmMonoInputStream extends InputStream implements Closeable
{
     ...
     * anyway. if the byte amount is not an order of bytes required for sample (such as 51 bytes left but 16 bit
     * samples)
    ...
    public int[] readSamplesAsIntArray(int amount) throws IOException
    {
        ...
        return Bytes.toReducedBitIntArray(bytez, readAmount, format.getBytePerSample(), format.getSampleSizeInBits(),
            format.isBigEndian());
    }
    ...
    public int[] readAll() throws IOException
    {
        ...
        return Bytes.toReducedBitIntArray(all, all.length, format.getBytePerSample(), format.getSampleSizeInBits(),
            format.isBigEndian());
    }
